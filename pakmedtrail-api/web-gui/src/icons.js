// Small inline SVG icon set. Each function returns an SVG string sized by `s`.
// Stroke uses currentColor so icons inherit text color. Kept lucide-ish so the
// look matches the original PharmaChain sidebar.

function svg(inner, s = 18, opts = "") {
  return `<svg class="ic" width="${s}" height="${s}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" ${opts}>${inner}</svg>`;
}
...
export const icons = {
  ...
  plus: (s) => svg('<path d="M5 12h14"/><path d="M12 5v14"/>', s),
  ...
  clock: (s) =>
    svg('<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>', s),
  box: (s) =>
    svg(
  ...
  ban: (s) =>
    svg('<circle cx="12" cy="12" r="10"/><path d="m4.9 4.9 14.2 14.2"/>', s),
};
